...
install_3proxy() {
    ...
    URL="https://github.com/hoangdoduy/proxyv6/raw/main/3proxy-0.9.4.zip"
    wget -qO- $URL | bsdtar -xvf-
    cd 3proxy-0.9.4
    make -f Makefile.Linux
    mkdir -p /usr/local/etc/3proxy/{bin,logs,stat}
    cp src/3proxy.o /usr/local/etc/3proxy/bin/
...
ulimit -n 65535
/usr/local/etc/3proxy/bin/3proxy.o /usr/local/etc/3proxy/3proxy.cfg
EOF
